...
def remove_handler_and_close(logger, handler):
...
# Not used, there is a single logger for the entire test file
# def get_logger_for_test_function(log_directory, log_all_events=True):
#     """instantiate the logger for the test execution.
#     log_directory:     where the file logs will be stored
#     console_log_level: the log level used for the console output
#     log_all_events:    log all the events or only golem events
...
#     # debug file log
#     log_file = os.path.join(log_directory, DEBUG_LOG_FILENAME)
#     file_handler_debug_ = logging.FileHandler(log_file, encoding='utf-8')
#     file_handler_debug_.setLevel(logging.DEBUG)
#     file_format_string = '%(asctime)s %(name)s %(levelname)s %(message)s'
#     file_formatter = logging.Formatter(file_format_string)
#     file_handler_debug_.setFormatter(file_formatter)
#     logger.addHandler(file_handler_debug_)
#     # info file log
#     log_file = os.path.join(log_directory, INFO_LOG_FILENAME)
#     file_handler_info_ = logging.FileHandler(log_file, encoding='utf-8')
#     file_handler_info_.setLevel(logging.INFO)
#     file_format_string = '%(asctime)s.%(msecs)03d %(levelname)s %(message)s'
#     file_formatter = logging.Formatter(file_format_string, "%H:%M:%S")
#     file_handler_info_.setFormatter(file_formatter)
#     logger.addHandler(file_handler_info_)
#     return logger, file_handler_debug_, file_handler_info_