...
def _parse_execution_data(execution_directory=None, project=None, suite=None,
                          execution=None, finalize=False):
    execution_data = execution_report_default()

    if not execution_directory:
        execution_directory = execution_report_path(project, suite, execution)
    ...
    for test in tests:  # test_file + test set
        ...
        for test_function in test_file_report:
            ...
            new_test_function['module'] = module
            test_name = test_split[-1]
            new_test_function['sub_modules'] = sub_modules
            new_test_function['name'] = test_name
            new_test_function['full_name'] = test

            # test_set_path = os.path.join(test_path, test_function)
            ...
            new_test_function['test_elapsed_time'] = test_function['test_elapsed_time']
            start_date_time = utils.get_date_time_from_timestamp(
                test_function['test_timestamp'])
            new_test_function['start_date_time'] = start_date_time
            new_test_function['browser'] = test_function['browser']
            new_test_function['data'] = test_function['test_data']
            new_test_function['environment'] = test_function['environment']
            # TODO, previous versions won't have set_name
            # remove the if when retro-compatibility is not required
            if 'set_name' in test_function:
                new_test_function['set_name'] = test_function['set_name']
...
def execution_report_path(project, execution_name, timestamp):
    return os.path.join(Project(project).report_directory_path, execution_name, timestamp)


def create_execution_directory(project, execution_name, timestamp):
    """Create the report directory for an execution.

    Directory should have the following path:
      <testdir>/projects/<project>/reports/<execution_name>/<timestamp>/
    """
    path = execution_report_path(project, execution_name, timestamp)
